...
<?php
    ...
    $u = $cnx->query("SELECT ABS(sum(montant)) FROM impaye WHERE code_impaye='01' AND num_utilisateur='".$_SESSION['num']."';");
    $un= $u->fetch();
    $d = $cnx->query("SELECT ABS(sum(montant))  FROM impaye WHERE code_impaye='02'AND num_utilisateur='".$_SESSION['num']."';");
    $deux= $d->fetch();
    $t = $cnx->query("SELECT ABS(sum(montant))  FROM impaye WHERE code_impaye='03'AND num_utilisateur='".$_SESSION['num']."';");
    $trois= $t->fetch();
    $q = $cnx->query("SELECT ABS(sum(montant))   FROM impaye WHERE code_impaye='04'AND num_utilisateur='".$_SESSION['num']."';");
    $quatre= $q->fetch();
    $c= $cnx->query("SELECT ABS(sum(montant))  FROM impaye WHERE code_impaye='05'AND num_utilisateur='".$_SESSION['num']."';");
    $cinq= $c->fetch();
    $s = $cnx->query("SELECT ABS(sum(montant))  FROM impaye WHERE code_impaye='06'AND num_utilisateur='".$_SESSION['num']."';");
    $six= $s->fetch();
    $ss = $cnx->query("SELECT ABS(sum(montant))  FROM impaye WHERE code_impaye='07'AND num_utilisateur='".$_SESSION['num']."';");
    $sept= $ss->fetch();
    $h = $cnx->query("SELECT ABS(sum(montant))  FROM impaye WHERE code_impaye='08'AND num_utilisateur='".$_SESSION['num']."';");
    $huit= $h->fetch();
?>
<html>
    ...
    <section class="container">
        ...
        <section>
            ...
            <div class="frame options">
                ...
                <a href="graphimpayes_utilisateur.php" class="option active">Somme des impayés</a>
        ...
        <canvas id="myChart">
        <script>
            const backgroundColorPlugin = {id: 'customBackgroundColor',beforeDraw: (chart) => {const ctx = chart.canvas.getContext('2d');ctx.save();ctx.fillStyle = '#272528';ctx.fillRect(0, 0, chart.width, chart.height);ctx.restore();}};
            Chart.register(backgroundColorPlugin);
            var xValues = ["Fraude à la carte", "Compte à découvert", "Compte clôturé","Compte bloqué","Provision insuffisante","Opération contestée par le débiteur","Titulaire décédé","Raison non communiqué, contactez la banque du client"];
            ...
            document.getElementById('exportPdf').addEventListener('click', async function () {
                ...
                const pdf = new jsPDF({orientation: "landscape"}); // Create a new PDF instance

                // Convert chart to Base64 image
                const chartImage = myChart.toBase64Image();

                // Add the image to the PDF
                pdf.addImage(chartImage, 'PNG', 10, 10, 190,190); // (image, format, x, y, width, height)
